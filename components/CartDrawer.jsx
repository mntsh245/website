...
export default function CartDrawer() {
  const { lines, totals, isOpen, closeCart, changeQuantity, removeLine, clearCart } =
    useCart();
  ...
  const buildWhatsAppLink = () => {
    const itemLines = lines
      .map(
        (line) =>
          `• ${line.name} (${line.portion === 'half' ? 'Half' : 'Full'} plate) × ${
            line.quantity
    ...
    const message = [
      ...
      '',
      `Type: ${mode === 'takeaway' ? 'Takeaway pickup' : 'Dine-in, first floor'}`,
      ...
      >
        <header className="flex items-center justify-between border-b border-line px-5 py-4">
          <div>
            ...
            <p className="text-xs text-muted">
              Nazirabad counter · {BRANCH.hours.replace('Daily, ', '')}
            </p>
          </div>
          <button
            type="button"
        ...
        {lines.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-8 text-center">
            <ShoppingBag size={30} className="text-accent" strokeWidth={1.4} />
            <p className="text-sm text-muted">
              Nothing here yet. Start with a half plate of galouti and one ulte tawe ka
              paratha — that is how most people begin.
            </p>
            <Link href="/menu" onClick={closeCart} className="btn-solid">
              Browse the menu
            </Link>
          </div>
        ) : (
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-xs text-muted">
                        {line.portion === 'half' ? 'Half plate' : 'Full plate'} ·{' '}
                        {line.unit} · {line.diet}
                      </p>
              ...
              <div className="mt-5 flex gap-2">
                {[
                  ...
                  { id: 'dinein', label: 'Dine-in, first floor' },
                ].map((option) => (
                  <button
                    key={option.id}
                    type="button"
            ...
            <footer className="border-t border-line bg-card px-5 py-4">
              <dl className="flex flex-col gap-1.5 text-sm">
                <div className="flex justify-between text-muted">
                  <dt>Subtotal</dt>
                  <dd className="text-fg">{currency(totals.subtotal)}</dd>
                </div>
                <div className="flex justify-between text-muted">
                  <dt>
                    Packaging ({lines.length} item{lines.length === 1 ? '' : 's'} ×{' '}
                    {currency(PACKAGING_PER_ITEM)})
                  </dt>
                  <dd className="text-fg">{currency(totals.packaging)}</dd>
                </div>
                <div className="flex justify-between text-muted">
                  <dt>GST at 5%</dt>
                  <dd className="text-fg">{currency(totals.gst)}</dd>
                </div>
                <div className="my-1 rule-gold" />
                <div className="flex justify-between text-base font-semibold">
                  <dt className="text-fg">Total payable</dt>
                  <dd className="text-accent">{currency(totals.grandTotal)}</dd>
                </div>
              </dl>
              ...
              <p className="mt-2 text-center text-[11px] text-muted">
                We confirm every order on WhatsApp before cooking. Pay at the counter on
                pickup.
              </p>
            </footer>
          </>
